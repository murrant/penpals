...
class NadAddressImport implements ToModel, WithChunkReading, WithBatchInserts
{
    ...
    public function model(array $row)
    {
        ...
        $address = new Address([
            ...
            'building' => ucwords(strtolower($row[24])),
            'floor' => ucwords(strtolower($row[25])),
            'unit' => ucwords(strtolower($row[26])),
            'room' => ucwords(strtolower($row[27])),
            'additional' => ucwords(strtolower($row[28])),
            'city' => ucwords(strtolower($city)),
            'county' => ucwords(strtolower($row[2])),
            'state' => $row[1],
            'zip' => $row[7],
            'zip4' => $row[8],
        ]);
